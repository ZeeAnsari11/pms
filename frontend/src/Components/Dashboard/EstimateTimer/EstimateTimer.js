...
const TimeEstimationField = ({ onHoursChange }) => {
  ...
  const handleInputChange = (e) => {
    ...
    if (!regex.test(inputVal) && inputVal !== "") {
      ...
      const [weeks = 0, days = 0, hours = 0, minutes = 0] = inputVal
        ...
        .split(/\s+/);
      const totalHours =
        parseInt(weeks) * 40 + parseInt(days) * 8 + parseInt(hours) + parseFloat(minutes) / 60;
      setHours(totalHours);
  ...
  return (
    <div>
      ...
      <input type="text" value={input} onChange={handleInputChange} style={inputStyles} placeholder={"2w 0d 6h 30m"}/>
      {error && <p>{error}</p>}
      {/*{hours && <p>{hours.toFixed(2)} hours</p>}*/}
    </div>
  );
};
